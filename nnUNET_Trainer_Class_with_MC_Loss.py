...
#import numpy as np
#
#class ABMUSoftDiceLoss_FP_FN(nn.modules.loss._Loss):
#    def __init__(self):
#        super(ABMUSoftDiceLoss_FP_FN, self).__init__()
#        self.ce = nn.CrossEntropyLoss(reduction='mean')
#    def forward(self, logits, target):
#
#        output = torch.argmax(logits,dim =1) 
#        
#        #critical_points_fg = target + output - 2 * target * output
#                
#        critical_points_fg = torch.logical_xor(target,output)
#        
#        FN = critical_points_fg*target
#        
#        new_in_crt  = logits*critical_points_fg
#        loss3 = self.ce(new_in_crt,FN[:,0].long())
#        
#        num_critical_points = len(torch.nonzero(critical_points_fg))
#        
#        return loss3*num_critical_points
#        
#        
#              
#abmu_loss = ABMUSoftDiceLoss_FP_FN() 
#my_term = .000005
...
def discretizer_softmax(data):
    # Apply softmax to the input data along dimension 1
    data = F.softmax(data, dim=1)
    return RoundSTE.apply(data)
...
class ABMUSoftDiceLoss_FP_FN(nn.modules.loss._Loss):
    ...
    def forward(self, logits, target):
        ...
        output = discretizer_softmax(logits)
        #critical_points_fg = target + output[:, [1]] - 2 * target * output[:, [1]]
        
        
        #output = torch.argmax(logits,dim =1) 
        #critical_points_fg = target + output - 2 * target * output
        
        critical_points_fg = torch.logical_xor(target,output[:, [1]])
...
class nnUNetTrainer(object):
    ...
    def train_step(self, batch: dict,epoch):
        data = batch['data']
        target = batch['target']
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            my_loss = abmu_loss(output, target)
            l = l1 + my_term*my_loss
...
#            
    ...
    def validation_step(self, batch: dict,epoch):
        data = batch['data']
        target = batch['target']
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                train_outputs.append(self.train_step(next(self.dataloader_train),epoch))
            ...
            with torch.no_grad():
                ...
                for batch_id in range(self.num_val_iterations_per_epoch):
                    val_outputs.append(self.validation_step(next(self.dataloader_val),epoch))
